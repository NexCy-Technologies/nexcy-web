        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-24">
          {teamMembers.map((member) => (
            <Card key={member.id} className="flex flex-col h-full !p-8 md:!p-12">
              <div className="text-6xl md:text-8xl font-mono text-muted/20 mb-8 tracking-tighter">
                {member.id}
              </div>
              <h3 className="text-2xl md:text-3xl font-mono uppercase font-bold text-foreground mb-3">
                {member.name}
              </h3>
              ...
              <div className="flex flex-wrap gap-3 mt-auto">
                {member.skills.map((skill) => (
                  <span 
                    key={skill} 
                    className="px-4 py-2 border border-[var(--border)] bg-transparent font-mono text-xs uppercase tracking-wider text-muted hover:text-foreground hover:border-foreground transition-colors cursor-default"
                  >
                    {skill}
                  </span>
                ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
            {values.map((value, i) => (
              <div key={i} className="p-6 md:p-8 border border-[var(--border)] bg-transparent">
                <h4 className="font-mono uppercase text-base font-bold mb-4">{value.title}</h4>
