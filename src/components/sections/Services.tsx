...
import { Globe, Smartphone, Bot, BarChart3, Code2, Cpu, Zap, Box } from "lucide-react";
...
function getServiceIcon(title: string) {
  ...
  if (t.includes("ai") || t.includes("machine") || t.includes("ml")) return Bot;
  if (t.includes("erp") || t.includes("pos")) return BarChart3;
  if (t.includes("software") || t.includes("dev")) return Code2;
  if (t.includes("iot") || t.includes("hardware")) return Cpu;
  if (t.includes("cloud")) return Zap;
  return Box;
}
...
export async function Services() {
  ...
  return (
    <section id="services" className="py-24 md:py-32 relative border-t border-[var(--border)] bg-background">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-t border-l border-[var(--border)] bg-background">
          {safeServices.map((service, index) => {
            ...
            return (
              <FadeUpStagger key={service.id} index={index}>
                <SpotlightCard
                  spotlightColor="rgba(0, 243, 255, 0.1)"
                  className="group relative h-full flex flex-col p-6 sm:p-8 rounded-none border-t-0 border-l-0 border-r border-b border-[var(--border)] bg-background transition-colors outline-none hover:ring-1 hover:ring-inset hover:ring-[var(--accent)] focus:ring-1 focus:ring-inset focus:ring-[var(--accent)] hover:z-10 focus:z-10"
                >
                  <div className="mb-4 text-[var(--accent)] relative z-10">
